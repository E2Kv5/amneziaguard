...
Ручная матрица (см. этап 7 ТЗ) — в первую очередь Samsung One UI: подключение к реальному AWG-серверу, сохранение состояния плитки при смене сети/перезагрузке, kill-switch (принудительный обрыв), DNS-leak self-test.
