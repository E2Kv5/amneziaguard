...
## Два датапаса и режим «без интернета»

Штатный `VpnService` не даёт API «отрезать приложение от сети», поэтому режим «без интернета» реализован собственным userspace-датапасом. Приложение выбирает путь автоматически при подключении:

| | Быстрый путь | Фильтрующий движок |
|---|---|---|
| Когда | нет правил «без интернета» | есть хотя бы одно такое правило |
| Как | tun отдаётся напрямую `amneziawg-go` — весь датапас в Go | наш `VpnService` + tun2socks: каждый поток релеится через SOCKS5 туннеля |
| Через VPN / напрямую | списки include/exclude интерфейса | то же + пофайловая фильтрация пакетов |
| Без интернета | ✗ (только root-ярус) | ✓ без root |
| Цена | — | датапас в JVM: медленнее и прожорливее |

Как движок решает, чей это поток: `ConnectivityManager.getConnectionOwnerUid` даёт UID владельца соединения, дальше UID → пакеты → правило. Для shared-UID берётся самое строгое правило. Именно из-за этого API минимальная версия — **Android 10**.

Что несёт движок: TCP и UDP (QUIC, DNS) — UDP уходит через SOCKS5 UDP ASSOCIATE, который amneziawg-go дозванивает netstack'ом WireGuard, то есть реально внутри туннеля. Если ассоциацию поднять не удалось, DNS переотправляется по TCP (RFC 7766), чтобы приложение не осталось без резолвера.

**Root-ярус** (`iptables` owner-match через libsu) остаётся как опция: он блокирует на уровне ядра независимо от состояния туннеля и не требует userspace-релея.
